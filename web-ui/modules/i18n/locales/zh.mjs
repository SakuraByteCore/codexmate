const zh = Object.freeze({
    ...
    'toolConfig.opencode.desc': '默认只读展示 OpenCode 配置；打开后才允许写入 XDG OpenCode 配置文件（如 ~/.config/opencode/opencode.jsonc）。',
    'toolConfig.opencode.lockedTitle': 'OpenCode 配置当前为只读',
    'toolConfig.opencode.lockedDesc': '这里不会写入 OpenCode 配置。要保存、导入或应用 provider/model，请先打开本 tab 的写入开关。',
    ...
    'opencode.providerStoreFile': 'CodexMate provider 库：{path}（添加多个 provider 只写这里，应用时才投影到 OpenCode）',
    ...
    'opencode.applyToCoreAgents': '同步应用到 build / plan / general / title / summary / compaction',
    ...
    'opencode.textarea.placeholder': '在这里展示/编辑 ~/.config/opencode/opencode.jsonc',
    'opencode.configFile.hint': '这里展示的是 OpenCode 生效配置；CodexMate 的多个 provider 草稿保存在独立 provider 库，避免污染 OpenCode 原生配置。',
